...
void clickCallback(const geometry_msgs::PoseStamped& msg) {     //虽然是pose类型的topic，但是旋转矩阵是单位阵，没有意义，且Z坐标都是0
  ...
  Eigen::Vector3d dir1 = (p2 - p1).normalized();   //测量距离并归一化
...
int main(int argc, char** argv) {
  ...
  click_sub_ = n.subscribe("/move_base_simple/goal", 10, clickCallback);//topic /move_base_simple/goal是由    rvizvisualisation  发布的
